...
module.exports = {
    createThanhVien: function (req, res) {
        ...
        return thanhvien
            .save()
            .then(() => {
                return res.redirect('/api/thanhviens')
            })
            .catch((error) => {
                console.log(error)
                res.status(500).json({
    ...
    updateThanhVien: function (req, res) {
        ThanhVien.findByIdAndUpdate({ _id: req.body.id }, { $set: req.body })
            .exec()
            .then(() => {
                res.redirect('/api/thanhviens')
            })
    },
    updateThanhVien = async (req, res) => {
        try {
          const blog = await blogService.updateThanhVien(req.params.id, req.body);
